...
public class SnappyFlowEsImpl extends SnappyFlow {
	 ...
	 */
	@Override
	public boolean transmitMetricData(HashMap<String, Object> payload) {
		logger.fine("Response Code : " + postRequest(new StringEntity(new JSONObject(payload).toString().replaceAll("=", ":"), ContentType.APPLICATION_JSON),false));
		return true;
	}
	@Override
	public boolean transmitEventData(HashMap<String, Object> payload){
		logger.fine("Response Code : " + postRequest(new StringEntity(new JSONObject(payload).toString().replaceAll("=", ":"), ContentType.APPLICATION_JSON),true));
		return true;
	}
